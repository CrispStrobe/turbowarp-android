...
/// Bluetooth Classic session via MFi ExternalAccessory (e.g. LEGO EV3, NXT with com.lego.les protocol).
class BTSession: ScratchLinkSession {
    ...
    private enum SessionState {
        case initial, discovery, connected
    }
    ...
    override func didReceiveCall(_ method: String, withParams params: [String: Any],
        ...
        case .initial:
            if method == "discover" {
                if let major = params["majorDeviceClass"] as? UInt,
                   let minor = params["minorDeviceClass"] as? UInt {
                    ...
                    completion(nil, JSONRPCError.invalidParams(
                        data: "majorDeviceClass and minorDeviceClass required"))
                }
                return
            }
        case .discovery:
    ...
    private func discover(inMajorDeviceClass major: UInt, inMinorDeviceClass minor: UInt,
        ...
        if !didRegisterForLocalNotifications {
            ...
            NotificationCenter.default.addObserver(
                forName: .EAAccessoryDidConnect, object: nil, queue: .main) { [weak self] _ in
    ...
    private func connect(toDevice deviceId: Int, completion: @escaping JSONRPCCompletionHandler) {
        guard let accessory = EAAccessoryManager.shared().connectedAccessories
                .first(where: { $0.connectionID == deviceId }),
            ...
            completion(nil, JSONRPCError.invalidRequest(data: "Device \(deviceId) not available"))
            return
        }
...
extension BTSession: BTStreamDelegate {
...
private class StreamDelegateHelper: NSObject, StreamDelegate {
    weak var delegate: BTStreamDelegate?
    func stream(_ aStream: Stream, handle eventCode: Stream.Event) {
        delegate?.stream(aStream, handle: eventCode)
    }
}

private protocol BTStreamDelegate: AnyObject {
    func stream(_ aStream: Stream, handle eventCode: Stream.Event)
}
